...
def generate_alert_hash(alert):
    # Hash on type + category + log_source ONLY.
    # Excludes: timestamp (changes each scan), description
    # (different log row picked each scan), event_id.
    # type + category + log_source = stable rule identity.
    fingerprint = (
        str(alert.get('type',        '')).strip() + '|' +
        str(alert.get('category',    '')).strip() + '|' +
        str(alert.get('log_source',  '')).strip()
    )
...
def clear_persistent_seen():
    """Call this when user clicks Reset on dashboard."""
    try:
        conn = _get_persistent_conn()
        conn.execute("DELETE FROM seen_alerts")
        conn.commit()
        conn.close()
        print("[SOC] Persistent alert history cleared")
    except Exception:
        pass
