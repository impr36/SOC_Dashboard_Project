...
class SOCLauncher:
    ...
    def _launch_dashboard(self):
        ...
        # Hide login window — poll for logout signal
        self.root.withdraw()
        self._poll_logout()

    def _poll_logout(self):
        """Poll every second for logout signal from the web dashboard."""
        if os.environ.get("SOC_LOGOUT_REQUESTED") == "1":
            # Stop the server
            global server_process
            if server_process:
                try:
            ...
            for k in ["SOC_LOGGED_USER", "SOC_LOGGED_ROLE",
                      "SOC_LOGGED_DISPLAY", "SOC_VIEWER_MODE",
                      "SOC_LOGOUT_REQUESTED"]:
                os.environ.pop(k, None)
            # Show login window again — do NOT destroy root
            self.root.deiconify()
            self._show_login_ui()
        else:
            self.root.after(1000, self._poll_logout)
...
def main():
    """Single launcher instance handles the full login/logout lifecycle.
    _poll_logout resets the UI on logout without destroying Tk root.
    """
    os.environ.pop("SOC_LOGOUT_REQUESTED", None)
    launcher = SOCLauncher()
    launcher.run()
